...
EOF
    ...
    if ! echo "$result" | grep -q "chars long\|minLength\|less than"; then
...
EOF
    ...
    if ! echo "$result" | grep -q "not supported\|enum\|invalid"; then
        echo "  ❌ 无效的 failurePolicy 未被拒绝"
        return 1
    fi
...
EOF
    ...
    if ! echo "$result" | grep -qi "required"; then
        echo "  ❌ envs 缺少 name 字段未被拒绝"
        return 1
    fi
    echo "  ✓ envs 缺少 name 字段被正确拒绝"

    # 测试7: 有效的 Sandbox 可以创建
...
EOF
    if ! kubectl get sandbox test-valid-sandbox -n "$TEST_NS" >/dev/null 2>&1; then
        echo "  ❌ 有效的 Sandbox 创建失败"
        return 1
    fi
